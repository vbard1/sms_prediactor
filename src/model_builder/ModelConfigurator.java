...
public class ModelConfigurator {
    private SmsXmlFile smsXmlFile;
    private String selectedAddress; // Variable pour stocker l'adresse sélectionnée
    ...
    public void launchTraining() {
        Document preparedXMLSources = prepareDocument();
        if (preparedXMLSources != null) {
            ...
            Main.log.severe("ModelConfigurator exited sooner than expected : preparedSources was null");
        }
    }
    ...
    private String chooseRecipientToTrainWith(Document preparedSources) {
        ...
        for (Map.Entry<Integer, String> entry : sortedAddresses.entrySet()) {
            ...
            JRadioButton radioButton = new JRadioButton("Adresse: " + address + ", " + occurrences + " sms");
    ...
    private Document prepareDocument() {
        try {
            ...
            Main.log.severe("Couldn't prepare file properly : " + smsXmlFile.filePath);
            return null;
        }
    }
    ...
    public void adressesStats(Document document) {
        ...
        for (int i = 0; i < smsNodes.getLength(); i++) {
            ...
            if (smsNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                smsXmlFile.adresses.put(address, smsXmlFile.adresses.getOrDefault(address, 0) + 1);
            }
        }
